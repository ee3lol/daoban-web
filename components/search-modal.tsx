...
export default function SearchModal({ isOpen, onClose }: SearchModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-[10vh] sm:pt-[15vh] px-4 backdrop-blur-[2px] bg-black/80">
      ...
      {/* Modal Container */}
      <div className="relative z-10 w-full max-w-2xl bg-[#050505] border border-white/10 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.8)] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
          <h2 className="text-xl font-bold text-[#EAE8E3] tracking-wide">Search</h2>
          <button 
            onClick={onClose}
        ...
        <div className="px-6 py-4 border-b border-white/5 bg-[#0a0a0a]">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full bg-[#151515] text-[#EAE8E3] border border-white/10 rounded-xl py-4 pl-12 pr-12 focus:outline-none focus:border-[#D47A73]/50 focus:ring-1 focus:ring-[#D47A73]/50 transition-all text-[15px]"
            />
            {query && (
              <button 
                onClick={() => setQuery('')}
        ...
        {/* Results Area */}
        <div className="flex-1 overflow-y-auto max-h-[60vh] custom-scrollbar bg-[#050505]">
          {isLoading && !results.length && (
            <div className="flex flex-col items-center justify-center p-12 text-white/30 gap-4">
              ...
              <p className="text-xs font-bold tracking-widest uppercase">Searching...</p>
            </div>
          )}
          ...
          {!query && (
            <div className="flex flex-col items-center justify-center p-16 text-white/20 gap-4">
              ...
              <p className="text-xs font-bold tracking-[0.2em] uppercase">Type to start searching</p>
            </div>
          )}
                    ...
                    <div className="flex items-center justify-between w-full sm:w-auto flex-1">
                      <div className="flex items-center gap-4 flex-1 min-w-0">
                        ...
                        <div className="flex flex-col justify-center min-w-0">
                          <h3 className="text-[#EAE8E3] font-bold text-[15px] truncate">{title}</h3>
                          <div className="flex items-center gap-2 mt-1 text-[11px] font-medium tracking-wider text-[#888888] uppercase">
                            ...
                            {item.vote_average ? (
                              <>
                                <span className="w-1 h-1 rounded-full bg-[#D47A73]/50" />
                                <span className="flex items-center gap-1 text-[#D47A73]">
                      ...
                      <button 
                        ...
                        className="sm:hidden p-2 text-white/40 hover:text-[#D47A73] transition-colors ml-2"
                      >
                        <ChevronDown className={`w-5 h-5 transition-transform duration-300 ${isExpanded ? 'rotate-180 text-[#D47A73]' : ''}`} />
                      </button>
                    </div>

                    {/* Actions Area */}
                    <div className={`${isExpanded ? 'flex pt-2 w-full' : 'hidden'} sm:flex items-center gap-2 w-full sm:w-auto opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity duration-300`}>
                      <Link
                        href={`/watch/${item.media_type}/${item.id}`}
                        onClick={onClose}
                        className="group/btn flex items-center justify-center gap-2 px-6 py-2.5 bg-[#D47A73] text-white hover:bg-[#DE867E] rounded-full font-bold text-[11px] tracking-widest uppercase transition-all duration-300 active:scale-95 flex-1 sm:flex-none"
                      >
