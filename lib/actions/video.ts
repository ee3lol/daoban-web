...
export async function fetchVideoSources(type: 'movie' | 'tv' | 'anime', tmdbId: number, season?: number, episode?: number) {
  try {
    ...
    const baseUrl = process.env.DAOBAN_API_URL || (process.env.NODE_ENV === 'production' ? 'https://api.daoban.lol' : 'http://localhost:3001');
    ...
    const response = await fetch(url, {
      ...
      // Important to skip caching for fast-changing streaming links
      cache: 'no-store'
    });
    ...
    return data; // Assuming it matches the expected JSON response
  } catch (error: any) {
